...
def main():
    ...
    # Wait for first successful connection — signalled by web_server via event
    ...
    while not (web_server.connection_params["connect_requested"]
               and web_server.connection_params["connect_success"]):
        time.sleep(0.5)
    ...
    mavlink_logger.info(f"Connection successful to {port} at {baud} baud")
    print("Total number of threads:", threading.active_count())

    # Wait for hardware validation to complete
